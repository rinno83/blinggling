...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 서비스 언어 목록</title>
	...
	<script>
		$(document).ready(function() {
			...
			$.ajax_paging = function(m_page) {
				...
				$.ajax({ 
					type: 'POST',
					url: "/service/language_paging",
					dataType: "json",
					data: 'current_page='+m_page+'&keyword='+keyword,
					error:function(data){
						console.log('language_paging fail');	
						console.log(data);
					},
						...
						if(result != undefined && result != null)
						{
							...
							if(result.result == 1)
							{
								...
								else
								{
									...
									{
										html += '<tr>';									
										html += '<td><input type="checkbox" class="unit" item_id="'+result.db_data[i]['lang_code']+'"/></td>';
										html += '<td>'+result.db_data[i]['lang_code']+'</td>';
			...
			$('#write_form_btn').click(function(){
				document.location.href = '/service/language_write_from';
			});
			...
			$('#delete_btn').click(function(){
				...
				{
					if(confirm('선택한 언어를 삭제하시겠습니까?'))
					{
						$.ajax({ 
							type: 'POST',
							url: "/service/language_delete",
							dataType: "json",
							data: 'item_ids='+item_ids,
							error:function(data){
								console.log('language_delete fail');	
								console.log(data);
							},
							success:function(result){
								if(result != undefined && result != null)
		...
		<div class="row">
			<div class="col-md-12">
				...
				<table class="table table-hover" id="table_list">
					...
					?>
						<tr>
							<td><input type="checkbox" class="unit" item_id="<?php echo $row['lang_code']; ?>"/></td>
