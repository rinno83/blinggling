...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 공지사항 수정</title>
	...
	<script>
		$(document).ready(function() {
			...
			$( 'textarea.ckeditor' ).val('<?php echo $notice['content']; ?>');
			
			$('#write_btn').click(function(){
			
				var m_notice_id = $('#notice_id').val();
				...
				{	
					$.ajax({ 
						...
						url: "/notice/modify",
						dataType: "json",
						data: 'notice_id='+m_notice_id+'&service_key='+m_service_key+'&service_id='+m_service_id+'&title='+encodeURIComponent(m_title)+'&content='+encodeURIComponent(m_content)+'&lang_code='+m_lang_code+'&is_show='+m_is_show,
						error:function(data){
						...
						success:function(result){
							...
							{
								alert('등록되었습니다.');
								window.location.href = '/notice?current_page='+m_current_page;
							}
							else
							{
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<div class="panel-body">
						...
						<form role="form" class="form-horizontal form-groups-bordered" enctype="multipart/form-data">
							...
							<input type="hidden" id="notice_id" value="<?php echo $notice['notice_id']; ?>" />
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['service_id']; ?>" service_key="<?php echo $row['key']; ?>" <?php if($notice['service_id'] == $row['service_id']){echo 'selected="true"';} ?>><?php echo $row['name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['lang_code']; ?>" <?php if($notice['lang_code'] == $row['lang_code']){echo 'selected="true"';} ?>><?php echo $row['lang_name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								...
								<div class="col-sm-5" id="typeDiv">
									<div id="label-switch" class="make-switch" data-on-label="ON" data-off-label="OFF">
										<input type="checkbox" id="is_show" <?php if($notice['is_show'] == 1){echo 'checked="true"';} ?>/>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="title" value="<?php echo $notice['title']; ?>">
								</div>
							</div>							
