...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - FAQ 수정</title>
	...
	<script>
		$(document).ready(function() {
			...
			$( 'textarea.ckeditor' ).val('<?php echo $faq['content']; ?>');
			
			$('#write_btn').click(function(){
			
				var m_faq_id = $('#faq_id').val();
				...
				{	
					$.ajax({ 
						...
						url: "/faq/modify",
						dataType: "json",
						data: 'faq_id='+m_faq_id+'&service_id='+m_service_id+'&service_key='+m_service_key+'&title='+encodeURIComponent(m_title)+'&content='+encodeURIComponent(m_content)+'&lang_code='+m_lang_code+'&is_show='+m_is_show,
						error:function(data){
						...
						success:function(result){
							...
							{
								alert('등록되었습니다.');
								window.location.href = '/faq?current_page='+m_current_page;
							}
							else
							{
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<div class="panel-body">
						...
						<form role="form" class="form-horizontal form-groups-bordered" enctype="multipart/form-data">
							...
							<input type="hidden" id="faq_id" value="<?php echo $faq['faq_id']; ?>" />
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['service_id']; ?>" service_key="<?php echo $row['key']; ?>" <?php if($faq['service_id'] == $row['service_id']){echo 'selected="true"';} ?>><?php echo $row['name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['lang_code']; ?>" <?php if($faq['lang_code'] == $row['lang_code']){echo 'selected="true"';} ?>><?php echo $row['lang_name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								...
								<div class="col-sm-5" id="typeDiv">
									<div id="label-switch" class="make-switch" data-on-label="ON" data-off-label="OFF">
										<input type="checkbox" id="is_show" <?php if($faq['is_show'] == 1){echo 'checked="true"';} ?>/>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="title" value="<?php echo $faq['title']; ?>">
								</div>
							</div>							
