...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 약관 목록</title>
	...
	<script>
		$(document).ready(function() {
			...
			$('#main-menu > li > ul > li:nth-child(4)').addClass('active');
						...
						if(result != undefined && result != null)
						{
							...
							if(result.result == 1)
							{
								...
								else
								{
									...
									{
										html += '<tr>';									
										html += '<td><input type="checkbox" class="unit" item_id="'+result.db_data[i]['terms_id']+'"/></td>';
										html += '<td>'+result.db_data[i]['terms_id']+'</td>';
										html += '<td>'+result.db_data[i]['lang_name']+'</td>';
										html += '<td>'+result.db_data[i]['type']+'</td>';
										html += '<td><a href="/terms/modify_form?terms_id='+result.db_data[i]['terms_id']+'&current_page='+result.page_data['page_info']['current_page']+'">'+result.db_data[i]['title']+'</td>';
			...
			$('#write_form_btn').click(function(){
				document.location.href = '/terms/write_from';
			});
		...
		<div class="row">
			<div class="col-md-12">
				...
				<table class="table table-hover" id="table_list">
					...
					?>
						<tr>
							<td><input type="checkbox" class="unit" item_id="<?php echo $row['terms_id']; ?>"/></td>
							<td><?php echo $row['terms_id']; ?></td>
							<td><?php echo $row['lang_name']; ?></td>
							<td><?php echo $row['type']; ?></td>
							<td>
								<a href="/terms/modify_form?terms_id=<?php echo $row['terms_id']; ?>&current_page=<?php echo $page_info['current_page']; ?>">
									<?php echo $row['title']; ?>
								</a>
							</td>
