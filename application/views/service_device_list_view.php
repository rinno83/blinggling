...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 서비스 다바이스 목록</title>
	...
	<script>
		$(document).ready(function() {
						...
						if(result != undefined && result != null)
						{
							...
							if(result.result == 1)
							{
								...
								else
								{
									...
									{
										html += '<tr>';									
										html += '<td><input type="checkbox" class="unit" item_id="'+result.db_data[i]['service_device_id']+'" device="'+result.db_data[i]['device']+'"/></td>';
										html += '<td>'+result.db_data[i]['service_device_id']+'</td>';
										html += '<td>'+result.db_data[i]['name']+'</td>';
										html += '<td><a href="/service_device/modify_form?service_device_id='+result.db_data[i]['service_device_id']+'&current_page='+m_page+'">'+result.db_data[i]['device']+'</td>';
										html += '<td>'+result.db_data[i]['update_date']+'</td>';
										html += '</tr>';
									}
			...
			$('#write_form_btn').click(function(){
				var m_service_id = $('#service_id').val();
				var m_service_name = $('#service_name').val();
				document.location.href = '/service_device/write_from?service_id='+m_service_id+'&service_name='+m_service_name;
			});
			...
			$('#delete_btn').click(function(){
				...
				$('.unit').each(function(){					
					...
					{
						item_obj = {
							'service_device_id' : $(this).attr('item_id'),
				...
				else
				{
					...
					if(confirm('선택한 디바이스를 삭제하시겠습니까?'))
					{
						$.ajax({ 
							type: 'POST',
							url: "/service_device/delete",
							dataType: "json",
							data: 'items='+encodeURIComponent(items)+'&service_id='+m_service_id,
							error:function(data){
...
<div class="page-container" style="min-height:100%;"><!-- add class "sidebar-collapsed" to close sidebar by default, "chat-visible" to make chat appear always -->	
	...
	<div class="main-content" style="min-height: 100%;">
		...
		<h2><?php echo $service_name; ?> 디바이스 목록</h2>
		<br/>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<input type="hidden" id="service_name" value="<?php echo $service_name; ?>" />			
				...
				<table class="table table-hover" id="table_list">
					...
					?>
						<tr>
							<td><input type="checkbox" class="unit" item_id="<?php echo $row['service_device_id']; ?>" device="<?php echo $row['device']; ?>" /></td>
							<td><?php echo $row['service_device_id']; ?></td>
							<td><?php echo $row['name']; ?></td>
							<td>
								<a href="/service_device/modify_form?service_device_id=<?php echo $row['service_device_id']; ?>&current_page=<?php echo $page_info['current_page']; ?>">
									<?php echo $row['device']; ?>
								</a>
							</td>
