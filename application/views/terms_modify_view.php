...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 약관 수정</title>
	...
	<script>
		$(document).ready(function() {
			...
			$( 'textarea.ckeditor' ).val('<?php echo $terms['content']; ?>');
			
			$('#write_btn').click(function(){
			
				var m_terms_id = $('#terms_id').val();
				...
				{	
					$.ajax({ 
						...
						url: "/terms/modify",
						dataType: "json",
						data: 'terms_id='+m_terms_id+'&service_id='+m_service_id+'&title='+encodeURIComponent(m_title)+'&content='+encodeURIComponent(m_content)+'&lang_code='+m_lang_code+'&is_show='+m_is_show+'&type='+m_type,
						error:function(data){
						...
						success:function(result){
							...
							{
								alert('등록되었습니다.');
								window.location.href = '/terms?current_page='+m_current_page;
							}
							else
							{
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<div class="panel-body">
						...
						<form role="form" class="form-horizontal form-groups-bordered" enctype="multipart/form-data">
							...
							<input type="hidden" id="terms_id" value="<?php echo $terms['terms_id']; ?>" />
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['service_id']; ?>" <?php if($terms['service_id'] == $row['service_id']){echo 'selected="true"';} ?>><?php echo $row['name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								...
								<div class="col-sm-2">
									...
									?>
										<option value="<?php echo $row['lang_code']; ?>" <?php if($terms['lang_code'] == $row['lang_code']){echo 'selected="true"';} ?>><?php echo $row['lang_name']; ?></option>
									<?php 
										endforeach;
									?>
							...
							<div class="form-group">
								<label class="col-sm-3 control-label">타입</label>
								
								<div class="col-sm-2">
									<select class="form-control" id="type">
										<option value="service" <?php if($terms['type'] == 'service'){echo 'selected="true"';} ?>>서비스 이용 약관</option>	
										<option value="private" <?php if($terms['type'] == 'private'){echo 'selected="true"';} ?>>개인정보 보호 정책</option>	
										<option value="info" <?php if($terms['type'] == 'info'){echo 'selected="true"';} ?>>서비스 이용 안내</option>										
									</select>
								</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5" id="typeDiv">
									<div id="label-switch" class="make-switch" data-on-label="ON" data-off-label="OFF">
										<input type="checkbox" id="is_show" <?php if($terms['is_show'] == 1){echo 'checked="true"';} ?>/>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control" id="title" value="<?php echo $terms['title']; ?>">
								</div>
							</div>							
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									...
									<div class="fileinput fileinput-new" style="text-align:right">
										<button type="button" class="btn btn-green" style="" id="makeUrlBtn">URL 만들기</button>
									</div>
