...
<html lang="en"><head>
	...
	<title>xenixstudio 서비스 플랫폼 - 공지사항 목록</title>
	...
	<script>
		$(document).ready(function() {
						...
						if(result != undefined && result != null)
						{
							...
							if(result.result == 1)
							{
								...
								else
								{
									...
									{
										html += '<tr>';									
										html += '<td><input type="checkbox" class="unit" item_id="'+result.db_data[i]['notice_id']+'"/></td>';
										html += '<td>'+result.db_data[i]['notice_id']+'</td>';
										html += '<td>'+result.db_data[i]['name']+'</td>';
										html += '<td>'+result.db_data[i]['lang_name']+'</td>';
										html += '<td><a href="/notice/modify_form?notice_id='+result.db_data[i]['notice_id']+'&current_page='+result.page_data['page_info']['current_page']+'">'+result.db_data[i]['title']+'</td>';
			...
			$('#write_form_btn').click(function(){
				document.location.href = '/notice/write_from';
			});
		...
		<div class="row">
			<div class="col-md-12">
				...
				<table class="table table-hover" id="table_list">
					...
					?>
						<tr>
							<td><input type="checkbox" class="unit" item_id="<?php echo $row['notice_id']; ?>" service_key="<?php echo $row['key']; ?>" lang_code="<?php echo $row['lang_code']; ?>"/></td>
							<td><?php echo $row['notice_id']; ?></td>
							<td><?php echo $row['name']; ?></td>
							<td><?php echo $row['lang_name']; ?></td>
							<td>
								<a href="/notice/modify_form?notice_id=<?php echo $row['notice_id']; ?>&current_page=<?php echo $page_info['current_page']; ?>">
									<?php echo $row['title']; ?>
								</a>
							</td>
