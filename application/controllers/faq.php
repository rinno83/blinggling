...
class Faq extends CI_Controller {
	...
	public function index()
	{
		...
		// DB에서 DATA 얻기
		$db_result = $this->faq_db_model->get_faq_list(0, '', $keyword, $offset, $limit);
		...
		//var_dump($view_data);
		$this->load->view('faq_list_view', $view_data);
	}
...
//////////////////////////////////////////////////////////////////////////////
	...
	public function write_from()
	{
		...
		$this->load->view('faq_write_view', $view_data);
	}
	...
	public function write()
	{
		...
		$db_result = $this->faq_db_model->set_faq(0, $service_id, $lang_code, $title, $content, $is_show);
		
		// write file cache
		write_file_cache('faq', $service_key, $lang_code);
...
//////////////////////////////////////////////////////////////////////////////
	...
	public function modify_form()
	{
		...
		$db_result = $this->faq_db_model->get_faq($faq_id);		
		if($db_result)
		{
			$db_result[0]['content'] = str_replace(array("\r\n", "\n"),"",$db_result[0]['content']);
			$view_data['faq'] = $db_result[0];
		}
		else
		{
			$view_data['faq'] = array();
		...
		$this->load->view('faq_modify_view', $view_data);
	}
	...
	public function modify()
	{
		...
		$db_result = $this->faq_db_model->set_faq($faq_id, $service_id, $lang_code, $title, $content, $is_show);
...
//////////////////////////////////////////////////////////////////////////////
	...
	{    	
		...
		{
			$db_result = $this->faq_db_model->del_faq($row->faq_id);
			
			// write file cache
			write_file_cache('faq', $row->service_key, $row->lang_code);
		}
...
// 		$params = $this->uri->uri_to_assoc(3, $default);
		...
		if(isset($_SERVER['HTTP_SERVICE_KEY']) && isset($_SERVER['HTTP_LANGUAGE_CODE']) && $current_page)
		{
			...
			if($service_id_array->result != "403")
			{
				...
				// service_key.faq File이 존재할 경우
				if($current_page == 1 && read_file($this->config->item('file_full_path').$service_key.'_'.$lang_code.'.faq'))
				{
					$string_data = read_file($this->config->item('file_full_path').$service_key.'_'.$lang_code.'.faq');
					$file_result_array = json_decode($string_data);
					
					foreach($file_result_array as $key => $row)
					{
						$faq_list[$key]['faq_id'] = $row->faq_id;
						$faq_list[$key]['title'] = $row->title;
						$faq_list[$key]['content'] = $row->content;
						$faq_list[$key]['regist_date'] = $row->regist_date;
					}
				...
				{
					// DB에서 DATA 얻기
					$db_result = $this->faq_db_model->get_faq_list($service_id, $lang_code, $keyword, $offset, $limit);
