		...
		neonLogin.$container.validate({
			...
			submitHandler: function(ev)
			{
				...
				$.ajax({
				  ...
				  dataType: "json",
				  data: { main_id: ev.username.value, main_pw:ev.password.value }
					...
					if(response.result == 1) {
						...
						if(response.result == 101) {							
							...
							$(password).closest('.input-group').addClass('validate-has-error');
							alert("패스워드가 잘못되었습니다.");
						}
					}
				});

				loadingTId = setTimeout(function()
							{
